...
<div class="mb-6">
  ...
  <h1 class="text-2xl font-bold mt-2">Edit Soal</h1>
</div>
  ...
  <form method="POST" action="{{ route('guru.questions.update', $question) }}">
    ...
    <div class="mb-4">
      ...
      <textarea name="pertanyaan" rows="3"
        class="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-blue-500 resize-none">{{ old('pertanyaan', $question->pertanyaan) }}</textarea>
    ...
    <div class="grid grid-cols-2 gap-4 mb-4">
      @foreach(['a'=>'Opsi A','b'=>'Opsi B','c'=>'Opsi C','d'=>'Opsi D'] as $key => $label)
        <div>
          ...
          <input name="opsi_{{ $key }}" value="{{ old('opsi_'.$key, $question->{'opsi_'.$key}) }}"
      ...
      <div>
        ...
        <select name="jawaban_benar" class="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm">
          @foreach(['a','b','c','d'] as $opt)
            <option value="{{ $opt }}" {{ old('jawaban_benar',$question->jawaban_benar)==$opt?'selected':'' }}>{{ strtoupper($opt) }}</option>
          @endforeach
        </select>
      </div>
      <div>
        <label class="block text-sm text-slate-400 mb-1">Tingkat</label>
